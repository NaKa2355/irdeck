...
// redux
import { clearDeleteRemoteStatus, clearPatchRemoteStatus, remoteSelected, remotesSelector, selectedRemoteIdSelector } from '../../ducks/remotes'
...
export const RemotesList: React.FC = React.memo(() => {
  ...
  const remotesListItems = remotes.map(remote => {
    return (<RemotesListItem selectedRemoteId={selectedRemote} key={remote.id} remote={remote} />)
  })
  return (
    <List>
      {remotesListItems}
